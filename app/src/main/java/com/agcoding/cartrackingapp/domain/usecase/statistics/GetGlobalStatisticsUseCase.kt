...
class GetGlobalStatisticsUseCase @Inject constructor(
    ...
    operator fun invoke(): Flow<GlobalStatistics> {
        return combine(
            ...
            // stored category name on every read rather than persisted, so it
            // applies to expenses that were saved long before this code.
            ...
            val perCarStatistics = cars.map { car ->
                ...
                val (carServiceExpenses, carOtherExpenses) = carExpenses.partition {
                    ExpenseCategories.isServiceCategory(it.category)
                }
