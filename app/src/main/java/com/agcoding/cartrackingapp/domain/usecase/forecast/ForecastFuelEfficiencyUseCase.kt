...
/**
 * Forecasts monthly fuel efficiency (L/100km) using Holt's Linear Exponential Smoothing.
 *
 * This use case:
 * - Aggregates fuel refills by month
 * - Calculates fuel efficiency for each month
 ...
 */
class ForecastFuelEfficiencyUseCase @Inject constructor(
    private val forecastEngine: HoltLinearSmoothingEngine
) {
    /**
     * Forecasts next month's fuel efficiency for a specific car.
     *
    ...
    operator fun invoke(refills: List<FuelRefill>): ForecastResult {
        ...
        // Extract efficiency values in chronological order
        val efficiencyValues = monthlyMetrics
            .sortedBy { it.month }
            .map { it.efficiency }
            .filter { it > 0.0 }  // Filter invalid values

        if (efficiencyValues.isEmpty()) {
            return ForecastResult(
                predictedNextValue = 0.0,
                trend = 0.0,
                confidence = 0.0,
        ...
        val filteredValues = forecastEngine.filterOutliers(efficiencyValues)
    ...
    /**
     * Aggregates fuel refills by month and calculates fuel efficiency.
     *
    ...
    private fun aggregateByMonth(refills: List<FuelRefill>): List<MonthlyFuelMetric> {
        ...
        }.filter { it.efficiency > 0 }  // Only include valid months
    }
}
